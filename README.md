# My-project
Welcome to my portfolio Website!
This project showcases my skills,  certificates, and the digital solution I have built as an aspiring Full-stack Developer.

## Live Demo
You can view the live website here:
https://riyamishra237.github.io/My-project/


## Features
* **Responsive Design:** Optimized for both Desktop and mobile views.
* **About me:** Detailed insights into my career goals and journey.
* **Certificates:** Showcasing my technical achievements.
* **Contact section:** Easy way to reach out for collaborations.

## Tech Stack
* **Tools:** VS Code, Git&GitHub

## Project Structure 
* 'index.html'- Home Page/ Landing section
* 'about.html' - career goals and skills
* 'certificate.html'- proof of technical skills
* 'contact.html'- Professional contact form

  ---
  ## Created by Riya Mishra
